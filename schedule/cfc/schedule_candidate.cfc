    ...
    <cffunction name="dataTable" access="remote" format="json">
        ...
        <cfquery datasource="#coldfusionDatasource#" name="qFiltered" result="myResult">
           SELECT  Distinct #listColumns_query#
            ...
            on J.JobVacancyId = R.JobId inner join CandidateDetails C on C.CandidateId = S.CandidateId inner join InterviewTypes IT on R.InterviewTypeId = IT.InterviewTypeId  left join Review Rw on R.InterviewTypeId = IT.InterviewTypeId where S.Status = 1 AND  S.Status = 1 
